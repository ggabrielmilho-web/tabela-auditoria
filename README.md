...
- **Pessoal** (folha de motoristas, `custo_pessoal`) — **pool por função, dividido por igual entre os veículos do tipo** (regra da diretoria, 24/09/2026), mês a mês (`_pessoal_por_cavalo`):

  | Função no RH | Vai para |
  |---|---|
  | Motorista Carreteiro | cavalos normais |
  | Motorista Carreteiro - 9 eixos | rodotrens — lista fixa `RODOTREM` (`QOX7H94`, `EWJ6C10`), porque o cadastro só conhece CAVALO/TRUCK |
  | Motorista de Caminhao Truck | trucks |
  | Motorista Carreteiro - inativo · a função genérica `Motorista` | **fora do custo** |

  O divisor é o nº de veículos **Rizza do tipo que rodaram Frota no mês** (os que têm linha na tela), não o cadastro: truck parado o mês inteiro não recebe, e o outro leva o pool inteiro (ago/26: OWH0F53 com R$ 22.248). Tipo sem nenhum veículo rodando no mês cai por igual em todos os que rodaram, para o total fechar. **Não casa nome**: motorista de 9 eixos que dirige cavalo normal continua no pool do rodotrem. Com vários meses marcados, cada mês divide o próprio pool e as fatias somam. Carreta não recebe folha; truck fica na visão Cavalo. **Provisão**: competência sem folha lançada usa a **anterior mais recente que tenha** — quando o RH lançar o mês, o real entra no lugar sozinho —, dividida pelos veículos que rodaram no mês pedido. Diagnóstico em `totais_custo`: `pessoal_total` (só o que entrou), `pessoal_fora` (inativos + genérica), `pessoal_pools` (valor e nº de veículos por mês e tipo), `pessoal_sem_veiculo_tipo`, `pessoal_provisionado` e `pessoal_meses_provisao` (ex.: `["2026-09<-2026-08"]`).
